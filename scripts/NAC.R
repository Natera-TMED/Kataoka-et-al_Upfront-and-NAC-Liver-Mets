...
all_data <- flexread("Galaxy Liver Mets Data_NAC ESMO GI2026_ver2.1.xlsx - NAC.tsv")
...
setnames(all_data, 
         old = c("date.of.start.of.ACT", "date.of.end.of.ACT", "PN"), 
         new = c("start.of.ACT", "Date.of.end.of.ACT", "pN"), 
         skip_absent = TRUE)
...
all_data[Mets.Type=='synchronous',Mets.Type:='Synchronous']

all_data[,.(Mets.Type,adjuvant.for.primary.tumor)] %>% table()
all_data[adjuvant.for.primary.tumor==FALSE & Mets.Type=='Metachronous',.(PtsID)]
...
cols_date <- c("Enrollment.Date","Surgery.Date", "Date.of.mets.surgery", 
               ...
               "DFS2.event.date","PFS2.event.date","start.of.ACT",
               "Date.of.end.of.ACT","Start.date.NAC","End.date.NAC")
...
## treatment end before start
all_data[Start.date.NAC>End.date.NAC]
all_data[End.date.NAC>start.of.ACT,.(PtsID,start.of.ACT,Date.of.end.of.ACT,End.date.NAC)]
all_data[start.of.ACT>Date.of.end.of.ACT,.(PtsID,start.of.ACT,Date.of.end.of.ACT)]

all_data[End.date.NAC>start.of.ACT,.(PtsID,start.of.ACT,Date.of.end.of.ACT,End.date.NAC)]

## treatment vs ref
all_data[End.date.NAC>Date.of.mets.surgery,.(PtsID,Date.of.mets.surgery,End.date.NAC)]
all_data[Start.date.NAC>Date.of.mets.surgery,.(PtsID,Date.of.mets.surgery,Start.date.NAC)]
all_data[start.of.ACT<Date.of.mets.surgery,.(PtsID,Date.of.mets.surgery,End.date.NAC)]
...
#############################
summary(all_data$ctDNA.MRD.Time) #68/7 =9.8
...
all_data[,PFS2_70Dlandmark:=((as.numeric(PFS2)*30)-70)/360*12]
...
all_data[!is.na(OS2.ESMO.GI),OS2.ESMO.GI_5Mlandmark:=OS2.ESMO.GI-5]
all_data[!is.na(PFS2),PFS2_5Mlandmark:=PFS2-5]
all_data[!is.na(OS2.ESMO.GI),OS2.ESMO.GI_27.7Mlandmark:=OS2.ESMO.GI-27.7]
all_data[!is.na(PFS2),PFS2_27.7Mlandmark:=PFS2-27.7]

ctDNA_3month_landmark <- max(all_data$ctDNA.3months.time, na.rm = TRUE)
ctDNA_3month_landmark
ctDNA_6month_landmark <- max(all_data$ctDNA.6months.time, na.rm = TRUE)
ctDNA_6month_landmark
all_data[,DFS_3MctDNAlandmark:=((as.numeric(DFS.ESMO.GI)*30)-ctDNA_3month_landmark)/360*12]
all_data[,DFS_6MctDNAlandmark:=((as.numeric(DFS.ESMO.GI)*30)-ctDNA_6month_landmark)/360*12]
all_data[,OS_3MctDNAlandmark:=((as.numeric(OS.ESMO.GI)*30)-ctDNA_3month_landmark)/360*12]
all_data[,OS_6MctDNAlandmark:=((as.numeric(OS.ESMO.GI)*30)-ctDNA_6month_landmark)/360*12]
...
all_sig_test=dt.events[cirID %in% all_data$PtsID & evType=='Test/ctDNA/Signatera' & evValue %in% c('POSITIVE','NEGATIVE'),.(cirID,evDate,evValue,MTM=evValue2)] 
all_sig_test=all_data[,.(cirID=PtsID,Date.of.mets.surgery,End.date.NAC,Start.date.NAC)][all_sig_test,on='cirID']
all_sig_test[,ref2test:=as.numeric(evDate-Date.of.mets.surgery)]
all_sig_test[,endNac2test:=as.numeric(evDate-End.date.NAC)]
all_sig_test[,endNac2ref:=as.numeric(Date.of.mets.surgery-End.date.NAC)]
unique(all_sig_test[,.(cirID,endNac2ref,End.date.NAC,Date.of.mets.surgery)][order(-endNac2ref)]) %>% head(100)
# all_sig_test[,startNac2test:=as.numeric(evDate-Start.date.NAC)]

all_sig_test[ref2test<=0] %>% dim()
all_sig_test[endNac2test<=0]
# all_sig_test[ref2test<=0][is.na(endNac2test)] 

all_sig_test[ref2test<=0 & endNac2test>0] %>% dim()
# all_sig_test[ref2test<=0 & endNac2test>28] %>% dim()

pre_surgican_data=all_sig_test[ref2test<=0 & endNac2test>0]
# pre_surgican_data=all_sig_test[ref2test<=0 & endNac2test>28]
...
all_data[pre_surgical_ctDNA=='NEGATIVE' & MTM!=0]
all_data[pre_surgical_ctDNA=='NEGATIVE', MTM:=0]


# View(pre_surgican_data)
all_sig_test

all_data[,cohort_def:='NAC']
all_data[,followup_drel:=(Date.last.FU-Date.of.mets.surgery)/30]

write.table(all_data,'NAC.tsv',quote=F,row.names=F,sep='\t')
###################
## select cohort ##
###################
...
print(doc,target='TableS3B_NAC_cohort_description.docx')
...
## DFS
circ_data <- all_data[all_data$pre_surgical_ctDNA!="",]
circ_data <- circ_data[circ_data$DFS_0Dlandmark>=0,]


survfit(Surv(time = circ_data$DFS_0Dlandmark, event = circ_data$DFS.Event)~pre_surgical_ctDNA, data = circ_data)
surv_object <-Surv(time = circ_data$DFS_0Dlandmark, event = circ_data$DFS.Event)
KM_curve <- survfit(surv_object ~ pre_surgical_ctDNA, data = circ_data,conf.int=0.95,conf.type="log-log") 
g=ggsurvplot(KM_curve, data = circ_data, pval = FALSE, conf.int = FALSE, risk.table = TRUE, break.time.by=6, palette=c("#46AC46","#004677"), title="DFS - ctDNA presurgical", ylab= "Disease-Free Survival", xlab="Time from Met surgery (Months)", legend.labs=c("ctDNA Negative", "ctDNA Positive"), legend.title="")
g
pdf('FigureS3A_NAC_preOp_DFS.pdf')
print(g, newpage = FALSE)
dev.off()
summary(KM_curve, times= c(24, 30, 36))
circ_data$pre_surgical_ctDNA <- factor(circ_data$pre_surgical_ctDNA, levels=c("NEGATIVE","POSITIVE"))
cox_fit <- coxph(surv_object ~ pre_surgical_ctDNA, data=circ_data) 
report_surv_extra(KM_curve, cox_fit)


## OS
circ_data <- all_data[all_data$pre_surgical_ctDNA!="",]
circ_data <- circ_data[circ_data$OS_0Dlandmark>=0,]

survfit(Surv(time = circ_data$OS_0Dlandmark, event = circ_data$OS.Event)~pre_surgical_ctDNA, data = circ_data)
surv_object <-Surv(time = circ_data$OS_0Dlandmark, event = circ_data$OS.Event)
KM_curve <- survfit(surv_object ~ pre_surgical_ctDNA, data = circ_data,conf.int=0.95,conf.type="log-log") 
g=ggsurvplot(KM_curve, data = circ_data, pval = FALSE, conf.int = FALSE, risk.table = TRUE, break.time.by=6, palette=c("#46AC46","#004677"), title="OS - ctDNA presurgical", ylab= "Overall Survival", xlab="Time from Met surgery (Months)", legend.labs=c("ctDNA Negative", "ctDNA Positive"), legend.title="")
g
pdf('FigureS3B_NAC_preOp_OS.pdf')
print(g, newpage = FALSE)
dev.off()
summary(KM_curve, times= c(24, 30, 36))
circ_data$pre_surgical_ctDNA <- factor(circ_data$pre_surgical_ctDNA, levels=c("NEGATIVE","POSITIVE"))
cox_fit <- coxphf(surv_object ~ pre_surgical_ctDNA, data=circ_data) 
report_surv_extra(KM_curve, cox_fit)
...
g
pdf('FigureS2C_NAC_Surv_DFS.pdf')
print(g, newpage = FALSE)
dev.off()
summary(KM_curve, times= c(24, 30, 36))
...
# circ_data <- all_data[!is.na(pre_surgical_ctDNA)]
# 
circ_data <- all_data[all_data$ctDNA.MRD!="",]
...
circ_data$pre_surgical_ctDNA <- factor(circ_data$pre_surgical_ctDNA, levels=c("NEGATIVE","POSITIVE"), labels = c("Negative", "Positive"))
...
## option
# cox_fit <- coxph(surv_object ~ pre_surgical_ctDNA + Age.Group + PrimSite + pT + pN + RAS + SizeLiverMetsmmGroup + NumLiverMetsGroup + Mets.Type + ACT, data=circ_data)
cox_fit <- coxph(surv_object ~ ctDNA.MRD + Age.Group + PrimSite + pT + pN + RAS + SizeLiverMetsmmGroup + NumLiverMetsGroup + Mets.Type + ACT, data=circ_data)
# cox_fit <- coxphf(surv_object ~ pre_surgical_ctDNA + Age.Group + PrimSite + pT + pN + RAS + SizeLiverMetsmmGroup + NumLiverMetsGroup + Mets.Type + ACT, data=circ_data)
...
g
pdf('Figure2C_NAC_MVA_MRD_DFS.pdf')
# pdf('Figure2D_NAC_MVA_MRD_OS.pdf')
print(g, newpage = FALSE)
dev.off()
...
g
pdf('Figure4A_NAC_ACT_ctDNAPos_DFS.pdf')
# pdf('Figure4B_NAC_ACT_ctDNANeg_DFS.pdf')
print(g, newpage = FALSE)
dev.off()
...
g
pdf('Figure4C_NAC_ACT_ctDNAPos_OS.pdf')
# pdf('Figure4D_NAC_ACT_ctDNANeg_OS.pdf')
print(g, newpage = FALSE)
dev.off()
...
g
pdf('FigureS4B_NAC_location.pdf')
print(g, newpage = FALSE)
dev.off()
